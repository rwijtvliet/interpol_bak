...
def triangles(anchorpoints:Iterable, anchorvalues:Iterable, valoutside=None) -> Callable:
    ...
    simp = Delaunay(anchorpoints) # each row has indices of the 3 anchorpoints that are the simplex corners.
    
    def interp(point):
        # Find simplex point is in.
        s = simp.find_simplex(point) #simplex-index that contains point. (-1 means point is in none)
        if s >= 0: #normal point, inside the hull
            # Get barycentric coordinates.
            b0 = simp.transform[s, :2].dot((point - simp.transform[s, 2]))
            weights = np.array([*b0, 1 - b0.sum()]) # add final coordinate / weight.
            indices = simp.simplices[s]
...
def polygon(anchorpoints:Iterable, anchorvalues:Iterable) -> Callable:
    """
    Generalised barycentric interpolation function for point in a polygon.
    
    Polygon has n vertices ("anchorpoints" of shape (n, 2)) in right-hand order
...
def interp_on_3axes(valuesA:Iterable, valuesB:Iterable, valuesC:Iterable,
    ...
    def polygon(*anchors):
        ...
        return {'shape': Delaunay(points), 'f': interp_func_polygon(points, values)}
